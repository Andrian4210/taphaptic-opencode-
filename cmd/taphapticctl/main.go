...
func run(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: taphapticctl <install-consumer|install-opencode|patch-settings|emit|health|smoke-local-e2e>")
	}
	...
	case "install-opencode":
		return runInstallOpencode(args[1:])
	case "patch-settings":
...
func runInstallOpencode(args []string) error {
	...
	apiBaseURL := fs.String("api-base-url", envOrDefault("TAPHAPTIC_API_BASE_URL", defaultAPIBaseURL), "API base URL")
	if err := fs.Parse(args); err != nil {
		return fmt.Errorf("usage: taphapticctl install-opencode [--api-base-url <url>]")
	}
	if fs.NArg() != 0 {
		return fmt.Errorf("usage: taphapticctl install-opencode [--api-base-url <url>]")
	}
	...
	if err := ensureDir(paths.OpencodePluginsDir, 0o755); err != nil {
		return err
	}

	if err := writeOpencodePlugin(paths.OpencodePluginPath, paths.InstalledCtlPath); err != nil {
		return err
	}
	...
	fmt.Printf("Installed Taphaptic opencode plugin at %s\n", paths.OpencodePluginPath)
	fmt.Println("Start a new opencode session so the plugin is loaded.")
...
func writeOpencodePlugin(destination, ctlBinaryPath string) error {
	plugin := opencodePluginScript(ctlBinaryPath)
	return writeFileAtomic(destination, []byte(plugin), 0o644)
}

func opencodePluginScript(ctlBinaryPath string) string {
	quoted := strconv.Quote(ctlBinaryPath)
	return `// Taphaptic opencode plugin - auto-generated by taphapticctl install-opencode
// Do not edit. Re-run: taphapticctl install-opencode
...
func runEmit(args []string) error {
	...
	source := fs.String("source", "", "event source (claude-code|opencode, default: claude-code)")
	if err := fs.Parse(args); err != nil {
		return fmt.Errorf("usage: taphapticctl emit --action <stop|subagent_stop|permission_prompt|idle_prompt|completed|subagent_completed|failed|attention> [--source <claude-code|opencode>]")
	}
	...
	if src := strings.TrimSpace(*source); src != "" {
		payload = applySource(payload, src)
	}
...
func resolveAppPaths() (appPaths, error) {
	...
	opencodePluginsDir := filepath.Join(homeDir, ".config", "opencode", "plugins")
	...
	return appPaths{
		...
		OpencodePluginsDir:    opencodePluginsDir,
		OpencodePluginPath:    filepath.Join(opencodePluginsDir, "taphaptic.js"),
	}, nil
}
...
// applySource replaces source-specific fields in a payload when a non-default
// source is specified (e.g. "opencode" instead of the default "claude-code").
func applySource(p eventPayload, source string) eventPayload {
	switch source {
	case "opencode":
		p.Source = "opencode"
		switch p.Type {
		case "completed":
			p.Title = "opencode completed"
			p.Body = "opencode completed a task"
		case "subagent_completed":
			p.Title = "opencode subagent completed"
			p.Body = "opencode subagent finished background work"
		case "failed":
			p.Title = "opencode failed"
			p.Body = "opencode reported a failure"
		case "attention":
			if strings.Contains(p.Body, "permission") {
				p.Title = "opencode needs permission"
				p.Body = "opencode is waiting for permission"
			} else {
				p.Title = "opencode needs attention"
				p.Body = "opencode needs attention"
			}
		}
	default:
		p.Source = source
